...
const PAYSTACK_PLANS = {
  [SubscriptionTier.CREATOR_PASS]:
    process.env.PAYSTACK_CREATOR_PASS_PLAN_CODE || "PLN_creator_pass",
  [SubscriptionTier.ENTERPRISE]:
    process.env.PAYSTACK_ENTERPRISE_PLAN_CODE || "PLN_enterprise",
} as const;
...
router.post(
  "/checkout",
  authenticate,
  fraudDetectionService.createPaymentFraudMiddleware(),
...
async function handlePaystackCheckout(
  ...
  try {
    ...
    const customer = await paystackService.createOrGetCustomer(userEmail, {
      userId,
    });
    ...
    const amount =
      tier === SubscriptionTier.CREATOR_PASS
        ? PAYSTACK_PRICING[SubscriptionTier.CREATOR_PASS]
        : PAYSTACK_PRICING[SubscriptionTier.ENTERPRISE];
    const planCode =
      tier === SubscriptionTier.CREATOR_PASS
        ? PAYSTACK_PLANS[SubscriptionTier.CREATOR_PASS]
        : PAYSTACK_PLANS[SubscriptionTier.ENTERPRISE];
    ...
    await fraudDetectionService.logPaymentAttempt(
      ...
      { fraudScore: 0, signals: [], recommendation: "approve" },
    ); // Low risk for checkout init
...
async function handleStripeCheckout(
  ...
  try {
    if (!process.env.STRIPE_SECRET_KEY) {
      return res
        .status(503)
        .json({ error: "Stripe payment processor not available" });
    }
    ...
    const priceId =
      priceIds[
        tier === SubscriptionTier.CREATOR_PASS ? "creator_pass" : "enterprise"
      ];
    if (!priceId) {
    ...
    logger.info("Stripe checkout session created (scaffolded)", {
      userId,
...
router.get(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      logger.error("Failed to get fraud risk", {
        error: error instanceof Error ? error.message : "Unknown error",
      });
