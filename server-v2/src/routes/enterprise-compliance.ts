...
router.post(
  ...
  async (req: Request, res: Response) => {
    try {
      const { type, organization, severity, title, message, metadata } =
        req.body;
...
router.post(
  ...
  async (req: Request, res: Response) => {
    try {
      const { organization, trustScore, complianceScore, kpiMetrics, trends } =
        req.body;
      ...
      if (
        ...
        return res.status(400).json({
          success: false,
          error:
            "Missing required fields: organization, trustScore, complianceScore",
        });
      }
      ...
      const success =
        await enterpriseIntegrationService.updateNotionKPIDashboard(kpiData);
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      const subscriptionId =
        await enterpriseIntegrationService.createWebhookSubscription({
          organizationId,
          endpoint,
          secret,
          events,
...
router.post(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      await enterpriseIntegrationService.deliverWebhook(
        event,
        organization,
        data,
      );
...
router.get("/dashboard", async (req: Request, res: Response) => {
  try {
    ...
    const integrationHealth =
      await enterpriseIntegrationService.checkIntegrationHealth();
