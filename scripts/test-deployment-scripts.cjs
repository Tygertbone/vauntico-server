...
// Logging functions
const log = (message) => {
    console.log(`${colors.blue}[${new Date().toISOString().slice(0, 19)}] ${message}${colors.reset}`);
};

const logSuccess = (message) => {
    console.log(`${colors.green}[${new Date().toISOString().slice(0, 19)}] ✅ ${message}${colors.reset}`);
};

const logWarning = (message) => {
    console.log(`${colors.yellow}[${new Date().toISOString().slice(0, 19)}] ⚠️  ${message}${colors.reset}`);
};

const logError = (message) => {
    console.log(`${colors.red}[${new Date().toISOString().slice(0, 19)}] ❌ ${message}${colors.reset}`);
};
...
const checkDependencies = () => {
    ...
    try {
        ...
        logWarning('curl not available (use PowerShell Invoke-WebRequest on Windows)');
    }
...
const testFileGeneration = () => {
    ...
    const tempDir = fs.mkdtempSync(path.join(require('os').tmpdir(), 'vauntico-test-'));
...
app.listen(PORT, '0.0.0.0', () => {
    console.log(\`Server running on port \${PORT}\`);
        ...
        fs.writeFileSync(path.join(tempDir, 'test_package.json'), JSON.stringify(packageJson, null, 2));
        ...
        runTest('Generated package.json valid', () => {
            const content = fs.readFileSync(path.join(tempDir, 'test_package.json'), 'utf8');
            JSON.parse(content);
        });
  ...
  apps: [{
    ...
    env: {
      ...
      PORT: 3000
        ...
        fs.writeFileSync(path.join(tempDir, 'test_ecosystem.config.js'), ecosystemConfig);
...
const testEnvironmentVariables = () => {
    ...
    runTest('Custom environment variables', () => {
        ...
        if (customPort !== '8080' || customNodeEnv !== 'development' || customLogDir !== '/tmp/test') {
...
const testDocumentation = () => {
    ...
    if (fs.existsSync('BACKEND_DEPLOYMENT_V2_GUIDE.md')) {
        runTest('Deployment guide exists', () => {
            ...
            if (!content.includes('# Vauntico Trust-Score Backend Deployment Guide')) {
        ...
        runTest('Deployment guide has sections', () => {
            const content = fs.readFileSync('BACKEND_DEPLOYMENT_V2_GUIDE.md', 'utf8');
            const requiredSections = ['## 🚀 What\'s New in v2.0', '## 🛠️ Prerequisites', '## 🚀 Quick Start'];
...
const main = () => {
    ...
    if (!fs.existsSync('backend-deploy-v2-optimized.sh')) {
        logError('Please run this script from the directory containing deployment scripts');
        process.exit(1);
    }
    ...
    const successRate = testsTotal > 0 ? Math.round((testsPassed / testsTotal) * 100) : 0;
    console.log(`${colors.blue}Success Rate: ${successRate}%${colors.reset}`);
    console.log('');
    
    if (testsFailed === 0) {
        logSuccess('🎉 All tests passed! The deployment scripts are ready for use.');
        console.log('');
