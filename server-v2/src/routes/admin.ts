  ...
  async (req: Request, res: Response): Promise<void> => {
    try {
      const limit = Math.min(
        parseInt(normalizeQueryParam(req.query.limit) ?? "") || 50,
        200,
      ); // Max 200
...
router.get(
  "/security/ip/:ip",
  requireAdmin,
  async (req: Request<{ ip: string }>, res: Response): Promise<void> => {
    try {
      const { ip } = req.params;
      const hours = parseInt(normalizeQueryParam(req.query.hours) ?? "") || 24;
...
router.post(
  ...
  async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      console.log(
        `Admin requested to block IP ${ip} for reason: ${reason ?? "No reason provided"}`,
      );
  ...
  async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      const success = await featureFlagManager.setFeatureFlag({
        ...
        description,
        percentage:
          type === FeatureFlagType.PERCENTAGE ? percentage : undefined,
        userIds: type === FeatureFlagType.USER_TARGETING ? userIds : undefined,
        environments:
          type === FeatureFlagType.ENVIRONMENT ? environments : undefined,
        createdBy: "admin",
      });
