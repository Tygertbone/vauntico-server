...
const API_KEYS = new Map<
  string,
  { tier: string; usageCount: number; lastUsed: Date }
>();

// Initialize with some demo API keys
API_KEYS.set("basic_key_xyz123", {
  tier: "basic",
  usageCount: 0,
  lastUsed: new Date(),
});
API_KEYS.set("pro_key_abc456", {
  tier: "pro",
  usageCount: 0,
  lastUsed: new Date(),
});
API_KEYS.set("enterprise_key_def789", {
  tier: "enterprise",
  usageCount: 0,
  lastUsed: new Date(),
});
...
export const checkRateLimit = async (
  ...
  if (
    !currentLimit ||
    now.getTime() - currentLimit.resetTime.getTime() > tierLimit.windowMs
  ) {
