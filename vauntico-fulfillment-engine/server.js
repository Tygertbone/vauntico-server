...
app.use((error, req, res, next) => {
  ...
  if (res.headersSent) {
    console.warn('⚠️ Headers already sent in error handler, delegating to default handler');
    return next(error);
  }
  ...
  try {
    const statusCode = (error && (error.status || error.statusCode)) ? (error.status || error.statusCode) : 500;
    ...
    try {
      ...
      console.error('❌ Final fallback failed in global error handler:', finalErr);
    }
  }
});
