...
router.post('/members', apiAuthMiddleware, async (req, res) => {
  try {
    ...
    res.status(500).json({
      success: false,
      error: error instanceof Error ? error.message : 'Failed to add council member'
    });
  }
});
...
router.get('/stats', apiAuthMiddleware, async (req, res) => {
  try {
    ...
    const completedProposals = await ubuntuCouncilService.getProposals('completed');

    const stats = {
      totalMembers: members.length,
      activeMembers: members.filter(m => m.isActive).length,
      totalVotingWeight: members.reduce((sum, m) => sum + m.votingWeight, 0),
      averageTrustScore: members.length > 0 
      ...
      memberDistribution: {
        ...
        mediumTrust: members.filter(m => m.trustScore >= 750 && m.trustScore < 850).length,
