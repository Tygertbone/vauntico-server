...
function TrustScoreDashboard() {
  ...
  } = useQuery({ queryKey: ["trustScore"], queryFn: fetchTrustScore });
  const {
    data: historyData,
    isLoading: isLoadingHistory,
    isError: isErrorHistory,
  } = useQuery({
    queryKey: ["trustScoreHistory"],
    queryFn: fetchTrustScoreHistory,
  });
  const {
    data: anomalyData,
    isLoading: isLoadingAnomalies,
    isError: isErrorAnomalies,
  } = useQuery({ queryKey: ["anomalies"], queryFn: fetchAnomalies });
  const {
    data: challengesData,
    isLoading: isLoadingChallenges,
    isError: isErrorChallenges,
  } = useQuery({ queryKey: ["challenges"], queryFn: fetchChallenges });
  const {
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      <header className="mb-8">
        ...
        <p className="text-gray-600">
          Your credibility, engagement, and legacy in one score
        </p>
              ...
              {anomalyData?.map((anomaly, index) => (
                <tr key={index}>
                  ...
                  <td className="py-2 px-4 border-b">
                    <span
                      className={`px-2 py-1 rounded-full text-xs font-semibold ${severityColors[anomaly.severity]}`}
                    >
                      {anomaly.severity}
                    </span>
                  </td>
          ...
          <ul className="space-y-2">
            {connectionsData?.map((connection, index) => (
              <li key={index} className="flex justify-between items-center">
                ...
                <span
                  className={`px-2 py-1 rounded-full text-xs font-semibold ${connection.connected ? "bg-green-100 text-green-800" : "bg-gray-100 text-gray-800"}`}
                >
