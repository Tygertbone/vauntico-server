...
class StandardsEnforcer {
  ...
  // Helper method to get exit code
  getExitCode() {
  ...
  async runAllChecks() {
    logInfo("🚀 Starting Vauntico Standards Enforcement...", "cyan");
    logInfo("=".repeat(50), "cyan");
    ...
    logInfo("=".repeat(50), "cyan");
    logInfo("📊 Standards Enforcement Summary:", "cyan");

    if (this.errors.length === 0 && this.warnings.length === 0) {
      logSuccess("🎉 All standards are compliant!");
      return 0;
    }
...
${colors.white}NPM Scripts:${colors.reset}
  ...
  ${colors.cyan}npm run standards:logger${colors.reset}        Run logger discipline
  ${colors.cyan}npm run standards:docs${colors.reset}          Run documentation
  ${colors.cyan}npm run standards:workflow${colors.reset}      Run workflow discipline
...
// Add enhanced methods to StandardsEnforcer class
Object.assign(StandardsEnforcer.prototype, {
  async runWeeklyScan(options) {
    logInfo("🗓️ Running Weekly Compliance Scan...", "cyan");
    logInfo("=".repeat(50), "cyan");
    ...
    for (const category of categories) {
      logInfo(`Scanning ${category}...`, "blue");
  ...
  async generateComplianceReport(options) {
    logInfo("📊 Generating Comprehensive Compliance Report...", "cyan");
    ...
    logSuccess(`Compliance report generated: ${reportPath}`);
    return 0;
  },

  async runComprehensiveScan(options) {
    logInfo("🔍 Running Comprehensive Standards Scan...", "cyan");
    logInfo("=".repeat(50), "cyan");
  ...
  async runQuickScan(options) {
    logInfo("⚡ Running Quick Standards Scan...", "cyan");
    ...
    if (this.errors.length === 0) {
      logSuccess("✅ Quick scan passed - No critical issues found");
    } else {
      logError(
  ...
  async runAutoFix(options) {
    logInfo("🔧 Running Auto-Fix for Common Issues...", "cyan");

    try {
      // Auto-fix ESLint issues
      logInfo("Running ESLint auto-fix...");
      execSync("cd server-v2 && npm run lint:fix", { stdio: "pipe" });
      logSuccess("ESLint auto-fix completed");

      // Auto-fix Prettier issues
      logInfo("Running Prettier auto-fix...");
      execSync("npm run format:fix", { stdio: "pipe" });
      logSuccess("Prettier auto-fix completed");

      logSuccess("🎉 Auto-fix completed successfully");
      return 0;
    } catch (error) {
...
`;
    ...
    logSuccess(`Weekly compliance report generated: ${reportPath}`);
  },
...
`;
    ...
    logSuccess(`Comprehensive standards summary generated: ${summaryPath}`);
  },
});
