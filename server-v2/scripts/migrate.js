...
async function migrate() {
  ...
  try {
    ...
    if (tableExists) {
      logger.success(
        "Migration has already been applied (users table exists). Skipping migration.",
      );
      const totalTime = Date.now() - startTime;
      logger.success(
        `Migration check completed successfully in ${totalTime}ms`,
      );
      return;
    }
    ...
    if (schemaCheck.rows[0].exists) {
      ...
      try {
        ...
        const migrationTime = Date.now() - migrationStart;
        logger.success(
          `Migration completed successfully in ${migrationTime}ms`,
        );
    ...
    const totalTime = Date.now() - startTime;
    logger.success(
      `Migration process completed successfully in ${totalTime}ms`,
    );
  } catch (error) {
    const totalTime = Date.now() - startTime;
    ...
    if (error.code === "ECONNREFUSED") {
      logger.error(
        "Connection refused - check database URL and network connectivity",
      );
