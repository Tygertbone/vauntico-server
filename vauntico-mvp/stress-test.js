...
export default function () {
  ...
  const trustResponse = http.post(
    `${BASE_URL}/trustscore/calculate`,
    trustScorePayload,
    {
      headers: {
...
export function setup() {
  logger.info(`Starting stress test of ${BASE_URL}`);

  // Verify endpoint is reachable
  const healthCheck = http.get(`${BASE_URL}/health`);
  if (healthCheck.status !== 200) {
    logger.error(
      `Health check failed: ${healthCheck.status} ${healthCheck.body}`
    );
