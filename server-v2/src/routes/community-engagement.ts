...
router.post('/engage', apiAuthMiddleware, async (req, res) => {
  try {
    ...
    res.status(500).json({
      success: false,
      error: error instanceof Error ? error.message : 'Failed to record community engagement'
    });
  }
});
...
router.post('/love-loops', apiAuthMiddleware, async (req, res) => {
  try {
    ...
    res.status(500).json({
      success: false,
      error: error instanceof Error ? error.message : 'Failed to create love loop'
    });
  }
});
...
router.post('/love-loops/:id/respond', apiAuthMiddleware, async (req, res) => {
  try {
    ...
    res.status(500).json({
      success: false,
      error: error instanceof Error ? error.message : 'Failed to respond to love loop'
    });
  }
});
...
router.post('/legacy-tree', apiAuthMiddleware, async (req, res) => {
  try {
    ...
    res.status(500).json({
      success: false,
      error: error instanceof Error ? error.message : 'Failed to add legacy tree entry'
    });
  }
});
...
router.post('/echo-chamber', apiAuthMiddleware, async (req, res) => {
  try {
    ...
    res.status(500).json({
      success: false,
      error: error instanceof Error ? error.message : 'Failed to create echo chamber story'
    });
  }
});
...
router.post('/echo-chamber/:id/publish', apiAuthMiddleware, async (req, res) => {
  try {
    ...
    res.status(500).json({
      success: false,
      error: error instanceof Error ? error.message : 'Failed to publish echo chamber story'
    });
  }
});
...
router.post('/leaderboards/:id/update', apiAuthMiddleware, async (req, res) => {
  try {
    ...
    const entry = await communityEngagementService.updateLeaderboardEntry(id, updateData);
    res.json({
      success: true,
      data: entry,
