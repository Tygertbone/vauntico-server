...
export default function AdminDashboard() {
  ...
  const adminKey =
    process.env.NEXT_PUBLIC_ADMIN_ACCESS_KEY || "default-admin-key";
  ...
  const handleSendUnlockCode = async () => {
    ...
    try {
      const response = await axios.post(
        "/api/admin/mfa/send-unlock-code",
        {
          userId: unlockUserId,
        },
        {
  ...
  const handleUnlockAccount = async () => {
    ...
    try {
      ...
      setUnlockStatus(
        error.response?.data?.error || "Failed to unlock account",
      );
    }
  };
        ...
        <div className="max-w-7xl mx-auto text-center">
          <div className="bg-white/90 backdrop-blur-sm rounded-lg p-8 shadow-lg">
            ...
            <p className="text-gray-600">
              Admin access key not configured in environment variables.
            </p>
          </div>
        </div>
      </div>
    ...
    <div className="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-100 p-6">
      <div className="max-w-7xl mx-auto">
        <div className="mb-8">
          ...
          <p className="text-gray-600">
            Monitor subscriptions, FX rates, and system analytics
          </p>
        </div>
        ...
        <div className="bg-white/90 backdrop-blur-sm rounded-xl p-6 shadow-lg mb-6 border border-white/20">
          ...
          {unlockStatus && (
            <div
              className={`mt-3 p-3 rounded-md ${unlockStatus.includes("success") ? "bg-green-100 text-green-700" : "bg-red-100 text-red-700"}`}
            >
              {unlockStatus}
            </div>
          )}
              ...
              {["ZAR", "NGN", "EUR"].map((currency) => (
                ...
                >
                  ...
                  <div className="text-2xl font-bold text-blue-600">
                    {fxRates.currency === currency
                      ? fxRates.rate.toFixed(4)
                      : "N/A"}
                  </div>
          ...
          {allProofs.length === 0 ? (
            <div className="text-center py-12 text-gray-500">
              ...
              <p className="text-sm mt-1">
                Proofs will appear here when users complete subscriptions.
              </p>
            </div>
          ) : (
