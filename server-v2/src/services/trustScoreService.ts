...
class MockTrustScoreDatabase {
  ...
  private userQuotas = new Map<
    string,
    { monthlyCalculations: number; creditsRemaining: number }
  >();
  ...
  async getTrustScore(
    ...
    const benefits =
      tierBenefits[tier as keyof typeof tierBenefits] || tierBenefits.basic;

    return {
      ...score,
      rateLimitRemaining:
        benefits.maxDailyRequests - (Math.floor(Math.random() * 10) + 1),
    ...
    setTimeout(
      async () => {
        try {
          ...
          console.log(
            `Trust score calculated for user ${userId}: ${currentScore.score} -> ${newScore}`,
          );
        } catch (error) {
  ...
  async checkCalculationQuota(
    ...
    const currentMonthCalculations = Array.from(
    ...
    if (
      currentMonthCalculations >= limits.maxCalculations ||
  ...
  async getTrustScoreHistory(
    ...
    const creditsUsed =
      scores.length * (tier === "basic" ? 1 : tier === "pro" ? 0.5 : 0.1);
...
export class TrustScoreService {
  ...
  static async calculateTrustScore(
    ...
    return await TrustScoreService.getInstance().calculateTrustScore(
      userId,
      tier,
    );
  }

  static async checkCalculationQuota(
    userId: string,
    tier: string,
  ): Promise<QuotaCheck> {
    return await TrustScoreService.getInstance().checkCalculationQuota(
      userId,
      tier,
    );
  }
  ...
  static async getTrustScoreHistory(
    ...
    return await TrustScoreService.getInstance().getTrustScoreHistory(
      userId,
      tier,
      limit,
      offset,
