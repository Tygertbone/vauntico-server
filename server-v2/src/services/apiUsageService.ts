...
class ApiUsageService {
  ...
  static async logUsage(
    ...
    console.log(
      `API Usage: ${userTier} tier accessed ${endpoint} endpoint at ${usageEntry.timestamp.toISOString()}`,
    );
  ...
  static async logWidgetUsage(
    ...
    console.log(
      `Widget Usage: ${usageData.action} action for user ${usageData.userId} at ${widgetUsageEntry.timestamp.toISOString()}`,
    );

    // Keep only last 1000 entries to prevent memory issues
    if (ApiUsageService.widgetUsageLog.length > 1000) {
      ApiUsageService.widgetUsageLog =
        ApiUsageService.widgetUsageLog.slice(-1000);
    }
  }
...
>>>>>>> origin/main
  ...
  static getUsageStats(tier: string): {
    ...
    const tierUsage = ApiUsageService.usageLog.filter(
      (entry) => entry.userTier === tier,
    );
  ...
  static async getWidgetMetrics(apiKey: string): Promise<WidgetMetrics> {
    const apiKeyUsage = ApiUsageService.widgetUsageLog.filter(
    ...
    const averageScore =
      apiKeyUsage.filter((entry) => entry.score !== undefined).length > 0
        ? totalScore /
          apiKeyUsage.filter((entry) => entry.score !== undefined).length
        : 0;

    return {
      actionCounts,
      activeWidgets: apiKeyUsage.filter((entry) => entry.action === "load")
        .length,
      tierDistribution,
      averageScore,
  ...
  static async getWidgetAnalytics(
    ...
    let filteredUsage = ApiUsageService.widgetUsageLog.filter(
      (entry) => entry.apiKey === apiKey && entry.timestamp >= cutoffTime,
    );

    // Filter by tier if specified
    if (options.tier !== "all") {
      filteredUsage = filteredUsage.filter(
        (entry) => entry.tier === options.tier,
      );
    }
    ...
    const scoreCount = filteredUsage.filter(
      (entry) => entry.score !== undefined,
    ).length;
    const averageScore = scoreCount > 0 ? totalScore / scoreCount : 0;
    const errorRate =
      filteredUsage.length > 0 ? errorCount / filteredUsage.length : 0;
  ...
  static async getTotalWidgetLoads(): Promise<number> {
    return ApiUsageService.widgetUsageLog.filter(
      (entry) => entry.action === "load",
    ).length;
  }
  ...
  static async getActiveWidgetCount(): Promise<number> {
    ...
    return ApiUsageService.widgetUsageLog.filter(
      (entry) => entry.action === "load" && entry.timestamp >= oneHourAgo,
    ).length;
  }

  static async getWidgetErrorRate(): Promise<number> {
    const totalUsage = ApiUsageService.widgetUsageLog.length;
    const errorCount = ApiUsageService.widgetUsageLog.filter(
  ...
  private static getRequestsByEndpoint(
    usage: UsageEntry[],
