...
router.post('/request', authenticate, paymentRequestValidation, async (req: Request, res: Response) => {
  try {
  ...
  const typedUserId: string = userId;
    ...
    const processingFeeCents = Math.round(amount * 0.10);
...
router.get('/requests', authenticate, paginationValidation, async (req: Request, res: Response) => {
  try {
    const userId = req.user?.userId;
    if (!userId) {
      return res.status(401).json({
...
router.post('/payout/:id', requireAdmin, async (req: Request, res: Response) => {
  try {
    const requestId = req.params.id;
    const adminUserId = req.user?.userId;

    // Validate request ID
    if (!requestId || !/^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/.test(requestId)) {
      return res.status(400).json({
        error: 'Bad Request',
    ...
    try {
      ...
      await query(`
        ...
        WHERE id = $2
      `, [payoutError instanceof Error ? payoutError.message : 'Unknown payout error', requestId]);

      // Send Slack alert for payout failure
      await sendSlackAlert(
        `❌ Payout Failed: ${paymentRequest.reference}`,
        {
          type: 'payout_failed',
          requestId,
          reference: paymentRequest.reference,
