...
export class EmailCampaignService {
  ...
  private async getTemplateData(userId: number): Promise<EmailTemplateData> {
    try {
      ...
      logger.error("Failed to get email template data", {
        userId,
  ...
  async sendCampaignEmail(
    ...
    try {
      ...
      if (userResult.rows.length === 0) {
        logger.warn("User not found for email campaign", {
          userId,
...
${process.env.FRONTEND_URL || "https://vauntico.com"}`;
      ...
      try {
        ...
        logger.error("Failed to log bounced email", {
          userId,
          campaignId: campaign.id,
          dbError,
        });
  ...
  async recordFeatureUsage(userId: number, featureName: string): Promise<void> {
    try {
      ...
      if (usageResult.rows.length > 0) {
        ...
        if (
          threshold &&
          usage.usage_count >= threshold &&
          !usage.threshold_triggered
        ) {
          // Trigger upgrade email
          await this.triggerUpgradeEmail(
            userId,
            featureName,
            usage.usage_count,
          );
   ...
   */
  private async triggerUpgradeEmail(
    userId: number,
    featureName: string,
    usageCount: number,
  ): Promise<void> {
...
Don't let limits slow you down! ✨`,
      ...
      logger.info("Limit approaching email sent", {
        userId,
        featureName,
        usageCount,
      });
