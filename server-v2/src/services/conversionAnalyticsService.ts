...
export class ConversionAnalyticsService {
  ...
  async trackConversionEvent(event: ConversionEvent): Promise<void> {
    try {
      ...
      if (
        ["upgrade_clicked", "payment_completed", "trial_started"].includes(
          event.eventType,
        )
      ) {
  ...
  async getPersonalizedUpgradePrompt(userId: string): Promise<{
    ...
    try {
      ...
      const positioningVariant = await this.assignUserToExperiment(
        userId,
        "upgrade_prompt_positioning",
      );
      const pricingVariant = await this.assignUserToExperiment(
        userId,
        "pricing_display",
      );
      ...
      if (score?.upgradeIntent && score.upgradeIntent > 0.7) {
        ...
        score.preferredFeatures.length > 0
      ) {
      ...
      return {
        ...
        features:
          preferredFeatures.length > 0
            ? preferredFeatures.slice(0, 3)
            : ["unlimited_ai", "advanced_analytics", "team_collaboration"],
    ...
    try {
      ...
      if (totalEvents > 0) {
        probability = Math.min(
          (positiveEvents / totalEvents) * 0.3 + probability,
          0.9,
        );
      }
   ...
   */
  private async calculateUserConversionScore(
    userId: string,
  ): Promise<UserConversionScore> {
    try {
  ...
  private async checkAndExecuteTriggers(
    ...
    try {
      ...
      for (const trigger of triggersResult.rows) {
        if (
          await this.shouldExecuteTrigger(userId, trigger, eventType, metadata)
        ) {
  ...
  private async shouldExecuteTrigger(
    ...
    if (
      ...
      trigger.trigger_key === "usage_limit_ai_generation"
    ) {
      return (
        metadata?.feature === conditions.feature &&
        metadata?.usagePercent >= conditions.threshold
      );
    }

    if (
      eventType === "trial_expiring" &&
      trigger.trigger_key === "trial_expiry_warning"
    ) {
  ...
  private async executeTrigger(userId: string, trigger: any): Promise<void> {
    try {
      ...
      if (actions.type === "upgrade_prompt") {
        ...
        await emailCampaignWorker.queueCampaignEmail(
          parseInt(userId),
          actions.template,
          actions,
        );
  ...
  private calculateUpgradeIntent(events: any[], isFreeUser: boolean): number {
    ...
    const upgradeEvents = events.filter((e) =>
      ["upgrade_clicked", "upgrade_prompt_shown"].includes(e.event_type),
    ).length;
  ...
  private calculatePriceSensitivity(events: any[]): string {
    const upgradeEvents = events.filter(
      (e) => e.event_type === "upgrade_clicked",
    ).length;
    const totalEvents = events.length;
  ...
  private generateSegmentTags(score: UserConversionScore): string[] {
    ...
    if (score.preferredFeatures.includes("team_collaboration"))
      tags.push("business_user");

    return tags;
  }

  private generatePersonalizedCTA(
    score: UserConversionScore | null,
    urgent: boolean,
  ): string {
  ...
  private async getUserConversionScore(
    userId: string,
  ): Promise<UserConversionScore | null> {
    try {
  ...
  private async sendPersonalizedUpgradePrompt(
    userId: string,
    actions: any,
  ): Promise<void> {
  ...
  private async sendPersonalizedOffer(
    userId: string,
    actions: any,
  ): Promise<void> {
...
export const conversionAnalyticsService =
  ConversionAnalyticsService.getInstance();
