---
day: 23
phase: monetization
title: "Digital Product Creation: Build It Once, Sell Forever"
duration: "90 minutes (includes creation time)"
objective: "Create your first digital product and set up for passive income"
---

# Day 23: Digital Product Creation 📘

## CREATE ONCE. SELL FOREVER.

**Affiliate income is great. But it depends on other people's products.**

**Digital products = YOU control everything:**

- ✅ 100% profit (no split with companies)
- ✅ Sell unlimited copies (no inventory)
- ✅ Passive income (sell while you sleep)
- ✅ Builds your brand (your product = your expertise)

**Today: Create your first digital product in 3-6 hours using only your phone.**

---

## 🎯 What is a Digital Product?

**Digital product = Something people download/access instantly:**

- Ebook/PDF guide
- Template (budget, planner, tracker)
- Video course/tutorial series
- Checklist or workbook
- Preset/filter pack (for content creators)
- Recipe collection
- Workout plan

**Delivered via:**

- WhatsApp
- Email
- Google Drive link
- Telegram

---

## 💡 Digital Product Ideas by Niche

### **Fitness:**

- 30-day home workout plan (R200-400)
- Meal prep guide with recipes (R150-300)
- Exercise form video library (R300-500)
- Progress tracker template (R100)

### **Fashion:**

- Capsule wardrobe guide (R150-300)
- "10 Outfits from 5 Pieces" lookbook (R200)
- Style quiz + personalized guide (R250-400)
- Fashion photo editing presets (R150)

### **Finance:**

- Budget template + tutorial (R100-250)
- "50 Ways to Save R500/Month" ebook (R150)
- Debt payoff calculator + guide (R200)
- Investment starter pack (R300-500)

### **Cooking:**

- 50 African recipes ebook (R200-400)
- Weekly meal prep plan (R150-250)
- Spice guide (R100-150)
- Budget-friendly recipe collection (R150)

### **Content Creation:**

- Video editing tutorial pack (R200-400)
- 100 viral hooks library (R150-250)
- Content calendar template (R100)
- Phone filming guide (R200)

---

## 📱 How to Create (Phone-Only)

### **Step 1: Choose Your Product Type**

**Beginner-friendly:** Start with a PDF guide/ebook

- Easiest to create
- Fastest to deliver
- Proven to sell

### **Step 2: Outline Your Content**

Open Google Docs, create outline:

**Example: "Budget Mastery Guide"**

```
Introduction (Why budgeting matters)
Chapter 1: Calculate Your Income
Chapter 2: Track Your Expenses (7-day challenge)
Chapter 3: Create Your Budget (Template included)
Chapter 4: 20 Money-Saving Hacks
Chapter 5: Stay Accountable (Habit tracker)
Bonus: Budget Template (fillable)
```

**Aim for:** 10-20 pages

### **Step 3: Write the Content**

**In Google Docs (phone app):**

- Write 1-2 pages per section
- Use bullet points (easier to read)
- Add personal examples
- Keep language simple
- Write like you talk (conversational)

**Time:** 2-4 hours

### **Step 4: Design It (Canva)**

**Download Canva app (free)**

**Create design:**

1. Open Canva → Search "Ebook"
2. Choose free template
3. Copy your text from Google Docs
4. Paste into template
5. Customize colors/fonts
6. Add your branding (name, logo if you have)
7. Export as PDF

**Time:** 1-2 hours

### **Step 5: Create Delivery System**

**Option A: Google Drive (Free)**

- Upload PDF to Google Drive
- Set sharing to "Anyone with link can view"
- Copy link

**Option B: Direct WhatsApp (Simple)**

- Send PDF directly after payment

**Option C: Gumroad (Professional)**

- Upload to Gumroad.com
- They handle payment + delivery
- Take 10% fee + Paystack fees

---

## 💰 How to Price Your Digital Product

### **Pricing Psychology:**

**Too cheap (R20-50):** "This must be low quality"  
**Too expensive (R1,000+):** "Too risky for my first purchase"  
**Sweet spot (R100-400):** "Affordable impulse buy, seems valuable"

### **Pricing by Product Type:**

**Checklists/Templates:** R50-150  
**Guides/Ebooks:** R150-300  
**Video courses:** R300-600  
**Bundles:** R400-800

### **Your First Product:** R150-250

- Low enough that people buy without overthinking
- High enough to be taken seriously
- Room for upsells later

### **The "Coffee Test":**

If your product costs less than 2 coffees (R100-150), it's an easy "yes" for most people.

---

## 🎯 Product Creation Template (Use This!)

### **"The Ultimate [Topic] Guide for [Audience]"**

**Structure:**

1. **Welcome Page:** Who this is for, what they'll learn
2. **Your Story:** Why you created this (builds trust)
3. **The Problem:** Pain point your audience feels
4. **The Solution:** Your method/system
5. **Step-by-Step Guide:** 5-10 actionable steps
6. **Resources:** Links, templates, bonus content
7. **Next Steps:** What to do after finishing guide

**Pages:** 12-20  
**Time to create:** 4-6 hours  
**Lifetime value:** Unlimited sales

---

## 📢 How to Launch Your Digital Product

### **Pre-Launch (Days before):**

**Day 1:** Tease it

- "I've been working on something for you..."
- Create curiosity

**Day 2:** Share the problem it solves

- "So many of you asked about [problem]. I created something to help."

**Day 3:** Show behind-the-scenes

- "Almost done! Here's a sneak peek..." (screenshot of content)

### **Launch Day:**

**Post 1: Announcement**

```
🎉 IT'S LIVE!

After [X] weeks of work, my [Product Name] is ready!

This is for you if:
✅ [Pain point 1]
✅ [Pain point 2]
✅ [Pain point 3]

Inside you get:
📘 [Feature 1]
📘 [Feature 2]
📘 [Feature 3]

Price: R[X] (Launch special - normally R[X+100])

DM me "GUIDE" to get it! 🔥

Only [X] copies at this price!
```

**Post 2: Social Proof**

- Share screenshots of early buyers
- Share testimonials (ask first buyers for feedback)

**Post 3: Reminder**

- "Last chance! Price goes up tomorrow!"

---

## 💳 How to Accept Payment (African-Friendly)

### **Option 1: Bank Transfer (Free)**

- Share bank details in DM
- They send proof of payment
- You send product

**Pros:** No fees  
**Cons:** Manual, slow

### **Option 2: PayPal (If Available)**

- Send PayPal.me link
- Instant notification
- Automatic

**Pros:** Professional  
**Cons:** Not everyone has PayPal in Africa

### **Option 3: Paystack Payment Link (BEST)**

- Create payment link at paystack.com
- Share link
- They pay via card/bank
- You get notified
- 1.5% + R2 fee

**Pros:** Professional, instant, trusted  
**Cons:** Small fee (worth it!)

### **Option 4: Yoco Payment Link**

- Similar to Paystack
- yoco.com/za/online-payments

### **Option 5: SnapScan/Zapper (South Africa)**

- QR code payment
- Instant
- Popular in SA

---

## 🌍 Real Example: Amara's Digital Product Launch

**Amara** (Lagos, Nigeria - Finance niche, 680 followers)

**Product:** "The Nigerian Money Blueprint" (R150)

**Creation:**

- Spent 6 hours writing in Google Docs
- Designed in Canva (2 hours)
- 18-page PDF guide
- Included budget template

**Pre-Launch (3 days):**

- Day 1: "I'm creating something for you..."
- Day 2: "Almost done! Who needs help with budgeting?"
- Day 3: "Launches tomorrow! Get ready!"

**Launch Day:**

- Posted at 6pm (peak time)
- "DM me MONEY to get it"
- 37 DMs in first 2 hours
- 19 sales that night = R2,850

**Week 1 total:** 52 sales = R7,800

**Week 2:** 26 more sales = R3,900

**Month 1 total:** R11,700 from ONE product

**Time invested:** 8 hours to create  
**Hourly rate:** R1,462.50/hour

**Her lesson:** "I was scared to charge R150. Now I wish I charged R250."

---

## ✅ Day 23 Action Checklist

- [ ] Chose my digital product idea
- [ ] Created outline (chapters/sections)
- [ ] Started writing content (at least 50% done)
- [ ] Downloaded Canva app
- [ ] Chose Canva template for design
- [ ] Decided on price (R150-300)
- [ ] Set up payment method (Paystack link or bank details)
- [ ] Planned launch date (within next 7 days)
- [ ] Shared in WhatsApp: "Day 23 ✅ Creating my FIRST DIGITAL PRODUCT! Passive income incoming! 📘💰"

---

## 🚀 Tomorrow: Day 24

**Service Offerings: High-Ticket Income**

Digital products = passive. Services = active (but HIGHEST income per sale).

Tomorrow you learn to package and sell your SERVICES.

**3 clients × R800 = R2,400/month from services alone.**

Let's add this income stream tomorrow! 💪

---

**See you tomorrow! 💪🌍**
