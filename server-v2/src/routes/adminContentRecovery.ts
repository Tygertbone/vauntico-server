...
router.post('/cases', requireAdmin, async (req, res) => {
  try {
    const { userId, stolenContentType, platform, stolenAmount, caseNotes } = req.body;

    // Validate required fields
    if (!userId || !stolenContentType || !platform || !stolenAmount || stolenAmount <= 0) {
      return res.status(400).json({
        success: false,
        error: 'Missing required fields: userId, stolenContentType, platform, stolenAmount (> 0)'
      });
    }
...
router.post('/cases/:caseId/update', requireAdmin, async (req, res) => {
  try {
    ...
    if (!['investigating', 'recovering', 'settled'].includes(status)) {
      return res.status(400).json({
        success: false,
        error: 'Invalid status. Must be "investigating", "recovering", or "settled"'
      });
    }
