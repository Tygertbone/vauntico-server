...
router.get('/api/v1/love-loops', async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    await ApiUsageService.logUsage(Array.isArray(apiKey) ? apiKey[0] : apiKey, 'love-loops-get', userTier);

    // Get love loops for user
    const loops = await communityEngagementService.getLoveLoopsForUser(userId);

    const response = {
      loops: loops,
      totalCredits: loops.reduce((sum, loop) => sum + (loop.creditsDonated || 0), 0),
      metadata: {
        version: '2.0.0',
...
router.post('/api/v1/love-loops', async (req: Request, res: Response) => {
  try {
    const { creatorId, supporterId, message, creditsDonated, isPublic } = req.body;
    const apiKey = req.headers['x-api-key'];
    ...
    // Log API usage with sacred naming
    await ApiUsageService.logUsage(Array.isArray(apiKey) ? apiKey[0] : apiKey, 'love-loops-create', userTier);
...
router.get('/api/v1/lore-generator', async (req: Request, res: Response): Promise<void> => {
  try {
    const userId: string = qp(req.query.userId);
    const apiKey = req.headers['x-api-key'];
    ...
    // Log API usage with sacred naming
    await ApiUsageService.logUsage(Array.isArray(apiKey) ? apiKey[0] : apiKey, 'lore-generator-get', userTier);

    // Get echo chamber stories (using existing service)
    const stories = await communityEngagementService.getEchoChamberStoriesForUser(userId);

    const response = {
      stories: stories,
      totalCredits: stories.reduce((sum, story) => sum + (story.creditsEarned || 0), 0),
      metadata: {
        version: '2.0.0',
...
router.post('/api/v1/lore-generator', async (req: Request, res: Response) => {
  try {
    ...
    // Log API usage with sacred naming
    await ApiUsageService.logUsage(Array.isArray(apiKey) ? apiKey[0] : apiKey, 'lore-generator-create', userTier);
...
router.get('/api/v1/ubuntu-echo', async (req: Request, res: Response): Promise<void> => {
  try {
    const userId: string = qp(req.query.userId);
    const apiKey = req.headers['x-api-key'];
    ...
    // Log API usage with sacred naming
    await ApiUsageService.logUsage(Array.isArray(apiKey) ? apiKey[0] : apiKey, 'ubuntu-echo-get', userTier);
...
router.post('/api/v1/ubuntu-echo', async (req: Request, res: Response) => {
  try {
    ...
    // Log API usage with sacred naming
    await ApiUsageService.logUsage(Array.isArray(apiKey) ? apiKey[0] : apiKey, 'ubuntu-echo-create', userTier);
...
router.get('/api/v1/credibility-circles', async (req: Request, res: Response): Promise<void> => {
  try {
    const userId: string = qp(req.query.userId);
    const apiKey = req.headers['x-api-key'];
    ...
    await ApiUsageService.logUsage(Array.isArray(apiKey) ? apiKey[0] : apiKey, 'credibility-circles-get', userTier);
...
router.get('/api/v1/narrative-engine', async (req: Request, res: Response): Promise<void> => {
  try {
    const userId: string = qp(req.query.userId);
    const apiKey = req.headers['x-api-key'];
    ...
    // Log API usage with enterprise naming
    await ApiUsageService.logUsage(Array.isArray(apiKey) ? apiKey[0] : apiKey, 'narrative-engine-get', userTier);
...
router.get('/api/v1/community-resonance', async (req: Request, res: Response): Promise<void> => {
  try {
    const userId: string = qp(req.query.userId);
    const apiKey = req.headers['x-api-key'];
    ...
    // Log API usage with enterprise naming
    await ApiUsageService.logUsage(Array.isArray(apiKey) ? apiKey[0] : apiKey, 'community-resonance-get', userTier);
