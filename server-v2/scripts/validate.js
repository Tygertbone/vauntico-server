...
async function validateDatabaseConnection() {
  ...
  try {
    ...
    addResult("Database Connection", "FAIL", errorMessage, guidance);
    return null;
  } finally {
    if (client) {
      client.release();
...
async function validateDatabaseSchema(client) {
  ...
  try {
    ...
    try {
      ...
      addResult("Database Size", "PASS", `Current database size: ${dbSize}`);
    } catch (sizeError) {
      addResult(
        "Database Size",
        "WARN",
...
async function validateMigrationFiles() {
  ...
  try {
    ...
    if (migrationFiles.length === 0) {
      addResult("Migration Files", "WARN", "No SQL migration files found");
      return;
    }
    ...
    for (const file of migrationFiles) {
      ...
      if (stats.size === 0) {
        addResult(`Migration File: ${file}`, "WARN", "Migration file is empty");
      } else {
        addResult(
          `Migration File: ${file}`,
          "PASS",
...
function printSummary() {
  ...
  const total =
    ...
    validationResults.warnings;
  const successRate =
    total > 0 ? Math.round((validationResults.passed / total) * 100) : 0;
  ...
  if (validationResults.failed === 0) {
    ...
    if (validationResults.warnings === 0) {
      ...
      console.log(
        `⚠️  ${validationResults.warnings} warning(s) - review recommended`,
      );
    }
  } else {
    console.log(
