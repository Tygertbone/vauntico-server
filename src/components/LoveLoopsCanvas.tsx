...
import { Heart, Palette, Share2, Download, Sparkles, Users, Lock, Unlock } from 'lucide-react'
...
const LoveLoopsCanvas: React.FC<LoveLoopsCanvasProps> = ({ className = '' }) => {
  ...
  const generateCollaborativeDoodle = useCallback(() => {
    ...
    const collaborativeDoodle: Doodle = {
      id: Date.now().toString(),
      points,
      color: abundanceColors[Math.floor(Math.random() * abundanceColors.length)],
  ...
  const addSacredSymbol = useCallback(() => {
    ...
    const centerX = canvas.width / 2
    const centerY = canvas.height / 2
    const symbol = sacredSymbols[Math.floor(Math.random() * sacredSymbols.length)]
          ...
          <div className="bg-white dark:bg-gray-800 rounded-2xl p-6 border border-gray-200 dark:border-gray-700">
            <div className="flex items-center justify-between mb-4">
              ...
              <div className="flex items-center space-x-2">
                ...
                >
                  {isCollaborative ? <Unlock className="w-5 h-5" /> : <Lock className="w-5 h-5" />}
                </button>
            ...
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center space-x-2">
                ...
                <input
                  ...
                  className="w-20"
                />
                <span className="text-sm text-gray-600 dark:text-gray-400 w-4">{brushSize}</span>
              </div>
        ...
        <div className="space-y-6">
          ...
          <div className="bg-white dark:bg-gray-800 rounded-2xl p-6 border border-gray-200 dark:border-gray-700">
            ...
            <div className="space-y-3">
              <div className="flex justify-between items-center">
                <span className="text-gray-600 dark:text-gray-400">Total Doodles</span>
                <span className="text-gray-900 dark:text-white font-semibold">{doodles.length}</span>
              </div>
              <div className="flex justify-between items-center">
                <span className="text-gray-600 dark:text-gray-400">Collaborative</span>
                <span className="text-gray-900 dark:text-white font-semibold">
                  {doodles.filter(d => d.author !== 'You').length}
                </span>
              </div>
              <div className="flex justify-between items-center">
                <span className="text-gray-600 dark:text-gray-400">Love Loops</span>
                <span className="text-gray-900 dark:text-white font-semibold">
                  {Math.floor(doodles.length * 0.7)}
                </span>
              </div>
              <div className="flex justify-between items-center">
                <span className="text-gray-600 dark:text-gray-400">Sacred Symbols</span>
                <span className="text-gray-900 dark:text-white font-semibold">
          ...
          <div className="bg-white dark:bg-gray-800 rounded-2xl p-6 border border-gray-200 dark:border-gray-700">
            ...
            <div className="space-y-2">
              {sharedDoodles.slice(-3).reverse().map((doodle, index) => (
                <div
                  key={doodle.id}
          ...
          <div className="bg-gradient-to-r from-pink-100 to-purple-100 dark:from-pink-900/20 dark:to-purple-900/20 rounded-2xl p-6 border border-pink-200 dark:border-pink-700">
            ...
            <p className="text-sm text-gray-700 dark:text-gray-300 italic">
              "Every stroke connects us, every color expresses our collective love. In this sacred space, we create abundance together."
            </p>
          </div>
        </div>
      </div>
