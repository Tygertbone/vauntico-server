...
if (process.env.STRIPE_SECRET_KEY) {
  ...
  logger.warn(
    "Stripe client not initialized - STRIPE_SECRET_KEY not found. Stripe routes disabled.",
  );
}
...
async function handleSubscriptionCancelled(
  ...
  try {
    ...
    if (!userId) {
      logger.error("No userId for cancelled subscription", {
        subscriptionId: subscription.id,
      });
      return;
    }
...
async function createOrUpdateSubscription(
  ...
  try {
    ...
    if (!subscriptionId) {
      logger.info("No subscription in checkout session (one-time payment?)", {
        sessionId: session.id,
      });
      return;
    }
