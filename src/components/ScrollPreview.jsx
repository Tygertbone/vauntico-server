...
const ScrollPreview = ({ scroll, hasAccess = false, onUpgrade }) => {
  ...
  const previewText =
    scroll.description ||
    scroll.excerpt ||
    "Unlock this scroll to reveal powerful insights...";
  const isLocked = !hasAccess;

  const handlePreviewClick = () => {
    if (isLocked) {
      trackScrollLockClick(
        scroll.id,
        scroll.title,
        scroll.tier || "pro",
        "free",
  ...
  return (
    <div className="card relative group hover:shadow-2xl transition-all">
      ...
      <div className={`relative ${isLocked ? "max-h-32 overflow-hidden" : ""}`}>
        <p className="text-gray-700 leading-relaxed mb-4">{previewText}</p>
      ...
      {isLocked ? (
        ...
        <button onClick={handlePreviewClick} className="btn-outline w-full">
      ...
      {isLocked && (
        <div className="mt-3 text-center">
          <p className="text-xs text-gray-500">
            Join {scroll.unlockedBy || "1,200+"} creators who unlocked this
            scroll
          </p>
        </div>
      )}
 ...
 */
export const ScrollPreviewGrid = ({
  scrolls,
  hasAccess = false,
  onUpgrade,
}) => {
 ...
 */
export const ScrollPreviewCarousel = ({
  scrolls,
  hasAccess = false,
  onUpgrade,
}) => {
