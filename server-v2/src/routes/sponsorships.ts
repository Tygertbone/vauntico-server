...
router.post("/", apiAuthMiddleware, async (req, res) => {
  try {
    ...
    const sponsorship =
      await sponsorshipService.createSponsorship(sponsorshipData);
    res.status(201).json({
      success: true,
      data: sponsorship,
...
router.put("/:id/kpis", apiAuthMiddleware, async (req, res) => {
  try {
    ...
    const sponsorship = await sponsorshipService.updateSponsorshipKPIs(
      id,
      kpis,
    );
    res.json({
    ...
    res.status(500).json({
      ...
      error:
        error instanceof Error
          ? error.message
          : "Failed to update sponsorship KPIs",
    });
  }
});
...
router.get("/:id/analytics", apiAuthMiddleware, async (req, res) => {
  try {
    ...
    const analytics = await sponsorshipService.getSponsorshipAnalytics(
      id,
      timeframe as any,
    );
    res.json({
