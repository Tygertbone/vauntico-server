...
class AlertManager {
  ...
  async sendAlert(data: AlertData): Promise<boolean> {
    ...
    if (
      ...
      data.severity === AlertSeverity.HIGH
    ) {
    ...
    if (now - lastSent < this.THROTTLE_DURATION) {
      logger.debug("Alert throttled", {
        alertKey,
        timeUntilNext: (this.THROTTLE_DURATION - (now - lastSent)) / 1000,
      });
      return false;
    }
  ...
  private async sendToSlack(data: AlertData): Promise<boolean> {
    ...
    try {
      ...
      logger.error("Failed to send Slack alert", {
        ...
        alertData: {
          ...data,
          details: data.details ? "[REDACTED]" : undefined,
        },
      });
      return false;
    }
  ...
  async alertDeploymentEvent(details: {
    ...
    await this.sendAlert({
      ...
      severity: details.status.includes("fail")
        ? AlertSeverity.HIGH
        : AlertSeverity.LOW,
...
// Export legacy API
export const sendSlackAlert = (message: string, details?: any) =>
  alertManager.sendSlackAlert(message, details);
