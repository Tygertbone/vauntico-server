...
router.post('/submit', authenticate, verificationRequestValidation, async (req: Request, res: Response) => {
  try {
    ...
    `, [userId, platform, platformUsername, platformUsername, generateVerificationToken(), 'pending', null, proofUrl, bio, 'api']);
...
router.get('/status/:id', authenticate, async (req: Request, res: Response) => {
  try {
    ...
    const now = new Date();
    if (verification.verification_status === 'pending' && verification.expires_at <= now) {
      await query(`
  ...
  try {
    ...
    const updatedVerification = await transaction(async (client) => {
      ...
      logger.info('Verification approved and trust score updated', {
        ...
        previousScore: updatedVerificationResult.rows[0]?.trust_score_impact || 0,
      });
